...
from ape_roll import client as ape_roll
...
def test_weiroll_planner_simple_program(alice, math):
    ...
    # TODO: hexconcat?
    assert commands[0] == ape_roll.hexConcat("0x771602f7000001ffffffffff", math.address)
...
def test_weiroll_with_return_value(alice, math):
    ...
    assert commands[0] == ape_roll.hexConcat("0x771602f7000001ffffffff01", math.address)
    assert commands[1] == ape_roll.hexConcat("0x771602f7000102ffffffffff", math.address)

    assert len(state) == 3
    assert state[0] == eth_abi.encode(["uint"], [1])
    assert state[1] == eth_abi.encode(["uint"], [2])
    assert state[2] == eth_abi.encode(["uint"], [3])
...
def test_weiroll_with_state_slots_for_intermediate_values(alice, math):
    ...
    assert len(commands) == 2
    assert commands[0] == ape_roll.hexConcat("0x771602f7000000ffffffff01", math.address)
    assert commands[1] == ape_roll.hexConcat("0x771602f7000001ffffffffff", math.address)
...
def test_weiroll_abi_encode(param, value, expected):
    ...
    literalValue = HexBytes(eth_abi.encode([param], [value]))
...
def test_weiroll_takes_dynamic_arguments(alice, strings):
    ...
    print(state)
    assert len(state) == 1
    assert state[0] == eth_abi.encode(["string"], [test_str])
...
def test_weiroll_returns_dynamic_arguments(alice, strings):
    ...
    assert len(state) == 2
    assert state[0] == eth_abi.encode(["string"], ["Hello, "])
    assert state[1] == eth_abi.encode(["string"], ["world!"])
...
def test_weiroll_takes_dynamic_argument_from_a_return_value(alice, strings):
    ...
    assert commands[1] == ape_roll.hexConcat(
...
def test_weiroll_supports_subplan(alice, math, subplanContract):
    subplanner = ape_roll.WeirollPlanner(alice)
    ...
    commands, state = planner.plan()
    assert commands == [
        ape_roll.hexConcat("0xde792d5f0082fefffffffffe", subplanContract.address)
    ]
    ...
    assert state[2] == ape_roll.hexConcat("0x771602f7000001ffffffffff", math.address)
...
def test_weiroll_return_values_across_scopes(alice, math, subplanContract):
    subplanner1 = ape_roll.WeirollPlanner(alice)
    sum = subplanner1.add(math.add(1, 2))

    subplanner2 = ape_roll.WeirollPlanner(alice)
    ...
    assert state[4] == ape_roll.hexConcat("0x771602f7000102ffffffffff", math.address)
